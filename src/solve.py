...
def processar_parte1(out_dir="out/metricas"):
    import os
    ...
    with open(os.path.join(out_dir, 'global.json'), 'w') as f:
    ...
    with open(os.path.join(out_dir, 'regioes.json'), 'w') as f:
    ...
    df_ego.to_csv(os.path.join(out_dir, 'ego_aeroportos.csv'), index=False)

    df_ego[['aeroporto', 'grau']].to_csv(os.path.join(out_dir, 'graus.csv'), index=False)

def processar_rotas(out_dir="out/metricas"):
    import os
    os.makedirs(out_dir, exist_ok=True)
    grafo = carregar_grafo('data/aeroportos_data.csv', 'data/adjacencias_aeroportos.csv')
    ...
    saida = os.path.join(out_dir, 'distancias_rotas.csv')
