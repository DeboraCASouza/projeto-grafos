...
export const Parte1Airports: React.FC<Parte1AirportsProps> = () => {
  ...
  return (
    <div className="flex w-full overflow-hidden animate-fade-in" style={{ height: 'calc(100vh - 48px)' }}>
      ...
      {/* ── Left Sidebar ── */}
      <aside
        className={`${sidebarOpen ? 'w-[380px]' : 'w-12'} shrink-0 bg-slate-950/85 backdrop-blur-md border-r border-slate-900 flex flex-col overflow-hidden transition-all duration-200 select-none`}
      >
        {/* Header */}
        <div className="p-3 border-b border-slate-900 flex items-center gap-2 shrink-0">
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 font-medium truncate">Parte 1 · Mapa Interativo</p>
            </div>
          )}
          <button
        ...
        {sidebarOpen && (
          <div className="flex flex-col flex-1 overflow-y-auto scrollbar-thin">
            {mapMode === 'shortest' ? (
              <div className="flex-1 flex flex-col">
                <div className="p-4 border-b border-slate-900 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Árvore de Percurso</label>
                ...
                <div className="p-4 border-b border-slate-900 flex flex-col gap-3">
                  <span className="text-[13px] font-bold text-slate-500 uppercase tracking-wider block">Rotas Obrigatórias</span>
                  <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-3 flex flex-col gap-1.5">
                    <div className="flex justify-between items-baseline">
                      ...
                      <span className="text-xs text-slate-500 font-mono">Custo: 3.62h</span>
                    </div>
                  ...
                  <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-3 flex flex-col gap-1.5">
                    <div className="flex justify-between items-baseline">
                      ...
                      <span className="text-xs text-slate-500 font-mono">Custo: 3.40h</span>
                    </div>
                    <div className="text-xs text-slate-300 font-medium">
                ...
                <div className="p-4 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Métricas do Subgrafo</label>
                  <div className="grid grid-cols-2 gap-1.5">
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                      ...
                      <span className="text-xs text-slate-500 font-semibold mt-1">Aeroportos (|V|)</span>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                      <span className="text-base font-extrabold text-purple-400 font-mono leading-none">2</span>
                      <span className="text-xs text-slate-500 font-semibold mt-1">Arestas (|E|)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="p-4 border-b border-slate-900 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Simulador de Rotas</label>
                  <p className="text-[13px] text-slate-400 leading-relaxed">
                ...
                <div className="p-4 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Métricas da Rede</label>
                  <div className="grid grid-cols-2 gap-1.5">
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                    ...
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                      ...
                      <span className="text-xs text-slate-500 font-semibold mt-1">Conexões (|E|)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <>
                <div className="p-3.5 border-b border-slate-900 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Busca por IATA</label>
                  ...
                  {searchQuery && (
                    <div className="text-xs text-slate-500 leading-relaxed min-h-[20px]">
                ...
                <div className="p-3.5 border-b border-slate-900 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Filtrar por Região</label>
                    ...
                    {regions.map((reg) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[13px] font-semibold">{reg}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="p-3.5 border-b border-slate-900 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Aeroporto Selecionado</label>
                    ...
                    {selectedInfo ? (
                      <>
                        <div className="flex items-baseline justify-between">
                          ...
                          <span className="text-[13px] font-semibold" style={{ color: REGION_COLORS[selectedInfo.regiao] }}>{selectedInfo.regiao}</span>
                        </div>
                        <div className="text-xs font-bold text-slate-300">{selectedInfo.cidade}</div>
                        <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs text-slate-500 mt-1 border-t border-slate-900/40 pt-1.5">
                      ...
                      <span className="text-xs text-slate-600 italic text-center">Clique em um aeroporto no mapa</span>
                    )}
                  </div>
                </div>

                <div className="p-3.5 flex flex-col gap-2">
                  <label className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Métricas Globais</label>
                  <div className="grid grid-cols-2 gap-1.5">
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                      <span className="text-base font-extrabold text-purple-400 font-mono leading-none">20</span>
                      <span className="text-xs text-slate-500 font-semibold mt-1">Aeroportos (|V|)</span>
                    ...
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                      ...
                      <span className="text-xs text-slate-500 font-semibold mt-1">Densidade global</span>
                    </div>
                    <div className="bg-slate-900/50 border border-slate-900/80 rounded-lg p-2 flex flex-col justify-between">
                      <span className="text-base font-extrabold text-emerald-400 font-mono leading-none">{visibleCount}</span>
                      <span className="text-xs text-slate-500 font-semibold mt-1">Visíveis (filtro)</span>
                    </div>
                  </div>
                </div>
              </>
            ...
            <div className="p-3 mt-auto text-xs text-slate-600 border-t border-slate-900/50 leading-relaxed font-semibold">
